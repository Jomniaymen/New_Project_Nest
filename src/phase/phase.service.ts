...
export class PhaseService {
  ...
  async createPhase(createPhaseDto: addphase) {
    ...
    for (let i = 1; i <= numberOfUnits; i++) {
      ...
      const unit = new this.unitModel({ Units:unitN,name: unitName, surfaceArea: unitSurfaceArea,numberline:numberLines});

      for (let j = 1; j <= numberLines; j++) {
        for (let k = 1; k <= pondLine; k++) {
          const pondName = `${unitName}-L${j}-P${k}`;
          const pond = new this.pondModel({ name: pondName, area: pondArea,pondline:pondLine});
        await pond.save()
 ...
 async phasedetailed(idphase:string){
...
const unitIds = units.map(unite => unite._id);


 await this.pondModel.deleteMany({_id:{$in:unitIds}})
